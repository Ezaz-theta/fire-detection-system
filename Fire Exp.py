...
def detect_live():
    ...
    detected_video=detector.detectObjectsFromVideo(camera_input=cap, frames_per_second=30, output_file_path=os.path.join(execution_path, "live-detected"), minimum_percentage_probability=40, log_progress=True )
    #cv2.imshow('Detected',output_image_path)
    #key=cv2.waitKey(10)
